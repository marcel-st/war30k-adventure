...
func get_move_speed_multiplier() -> float:
	if _cooldowns.is_empty():
		return 1.0
	var surge_remaining: float = float(_cooldowns.get("resilience_surge", 0.0))
	if surge_remaining > 0.0:
		return 1.08
	var rally_remaining: float = float(_cooldowns.get("rally_command", 0.0))
	...
	match effect_type:
		"damage_reduction":
			if _owner.has_method("apply_temporary_damage_reduction"):
				_owner.apply_temporary_damage_reduction(float(effect.get("ratio", 0.2)), float(effect.get("duration", 3.0)))
		"aoe_dot":
